...
- [データ設計（論理構成・詳細）](#データ設計論理構成詳細)
  ...
  - [5. 各エンティティ詳細](#5-各エンティティ詳細)
    ...
    - [6 口座取引・資産管理](#6-口座取引資産管理)
      ...
      - [6.2account\_balances\_snapshots](#62account_balances_snapshots)
...
#### 6.2account_balances_snapshots

「大量の取引データを毎回全件参照するのは限界がある」という課題、非常によく理解できました。

計算の**「チェックポイント（基点）」**を作るイメージですね。膨大な履歴を遡らなくても、特定の日の「繰越データ」さえ見れば、そこから先の数件の取引を足すだけで現在の資産が計算できる、という設計が最適です。

これを実現するための、スナップショット（締め）に特化したテーブル構造を提案します。


このテーブルは、`account_transactions` の計算負荷を減らすための**「ショートカット」**として機能します。
...
| **as_of_date** | date | NO | 締日の日付（例：2023-12-31）。 |
| **balance_type** | text | NO | `CASH`（現金残高） / `MARKET_VALUE`（時価評価額合計） / `CREDIT_PRINCIPAL`（信用建玉代金合計）。 |
| **amount** | numeric | NO | **その時点の確定金額。** |
| is_verified | boolean | NO | 証券会社のデータと照合済みならTRUE（信頼の基点）。 |
| is_manual | boolean | NO | True 手入力、False 自動計算。 |
| memo | text | YES | メモ|
...
これまでは、現在の資産を知るために「サービス利用開始時からの全データ」を `SUM` する必要がありました。このテーブルを導入すると、計算式が以下のように劇的に変わります。

**【従来の計算】**
全期間の amount 合計 = 現在の資産


**【新しい計算（今回の提案）】**
直近の snapshots.amount + ( snapshots.as\_of\_date 以降の transactions.amount 合計 ) = 現在の資産

##### 運用のメリット

1. **参照範囲の限定:** `transaction_date > (直近の締日)` のデータだけを見れば良いため、数年分のデータがあっても直近1ヶ月分程度の参照で済みます。
2. **整合性の担保:** 万が一、過去の `transactions` を修正したり消してしまったりしても、この `snapshots` の数値が「正解」として固定されているため、全体の集計が壊れるのを防げます。
3. **多目的利用:** `balance_type` を分けることで、「現金残高の繰越」と「信用建玉の繰越（未決済金額）」を同じ構造で管理でき、拡張性が高いです。
